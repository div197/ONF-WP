#!/bin/sh
# ONF-WP Entrypoint Script v1.0.3
...
log_info() {
    echo "[INFO] $(date '+%Y-%m-%d %H:%M:%S') - $1" >&1
}
...
if [ ! -f "${WP_CONFIG_PATH}" ] && [ -f "${WP_CONFIG_SAMPLE_PATH}" ]; then
  log_info "wp-config.php not found. Configuring from sample..."
...
log_info "ONF-WP initialization complete. Starting PHP-FPM..."
log_info "Container ready for WordPress requests"

# Execute PHP-FPM via Wodby's entrypoint for proper initialization
